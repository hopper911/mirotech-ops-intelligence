...
export default function ExecutivePage() {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8">
      ...
      <RevealStagger className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" stagger={0.07}>
        {dash.kpis.map((kpi) => (
          <RevealItem key={kpi.id}>
            <div
              className={`glass-app h-full rounded-2xl p-4 ${
      ...
      <Reveal delay={0.1} className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
            ...
            {dash.topRecommendations.map((rec) => (
              <RevealItem key={rec.id}>
                <Link
                  ...
                  className="glass-app glass-lift block h-full rounded-2xl p-5"
                >
